...
const EMAIL_RECIPIENT = process.env.EMAIL_RECIPIENT || 'recipient@example.com';
...
async function createTransporter() {
  if (!transporter) {
    if (GMAIL_USER && GMAIL_PASS) {
      ...
      console.log('No Gmail credentials found, using Ethereal for testing');
...
app.post('/api/send-email', async (req, res) => {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: '"Demo Request" <' + (process.env.MAIL_FROM || GMAIL_USER || 'noreply@example.com') + '>',
      to: EMAIL_RECIPIENT,
